...
function FileProgress(file,fileListID,swfUploadInstance) { 
	if(!document.getElementById(file.id)){
		...
		var fileNamesplit=file.name.split(".")[0]; 
		fileNameNonumber=fileNamesplit.replace('-',''); 
			//文件名不为数字
			if(isNaN(fileNameNonumber)||file.name.split(".").length>2){
				swfUploadInstance.cancelUpload(file.id);
				var tr = document.getElementById(file.id);
				tr.style.color="red";
				var bar = document.getElementById(file.id+"_bar");
			...
			//文件名不在拍品序号中存在
			for(o=0; o<onumberstrs.length;o++){ 
				if(fileNamesplit.split("-")[0]==onumberstrs[o]){
					judgeStr=1;	
				}
			}
			if(judgeStr==0){
