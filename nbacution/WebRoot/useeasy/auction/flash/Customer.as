package {
	...
	public class Customer extends MovieClip {
		...
		private var apply_num:String; 
		private var xml_num:String;
		private var auction_name:String;
		private var user_name:String;
		private var bid_num:String;
		private var margin_total_val:String;
		private var margin_val:String;
		private var team_str_val:String;
		private var team_str_array:Array;
		private var proxy_str_val:String;
		private var proxy_str_array:Array;
		private var proxy_value:String;
		private var price_authority:String;
		...
		private var dataGridFormat:TextFormat = new TextFormat("宋体",14,0x666666);
		private var dataGridSpecialFormat:TextFormat = new TextFormat("宋体",14,0xff0000);
		private var priceDataProvider:DataProvider = new DataProvider();
		private var dealDataProvider:DataProvider = new DataProvider();
		
		//var dealAlertBox:DealAlertBox;
		private var priceAlertBox:PriceAlertBox;
		private var connectCloseAlertBox:ConnectCloseAlertBox;
		...
		private function connectHandler(e:Event):void{
			...
			if(param["margin_total_val"] != null){
				margin_total_val =  param["margin_total_val"];
			}else{
				margin_total_val = "-";
			}
			if(param["proxy_str_val"] != null){
				proxy_str_val = param["proxy_str_val"];
				proxy_str_array = proxy_str_val.split("#");
			}else{
				proxy_str_val = "-1#";  
				proxy_str_array = proxy_str_val.split("#");
			}
		...
		public function startLoadXml(e:TimerEvent):void{
			...
			sendMessage("90", (auction_id + "&" + account_id + "&gs&" + apply_num + "&"));
		}
			...
			while( line.indexOf("ues_t_a") != -1 && line.indexOf("uee_n_d") != -1 
				...
				if( line_length != new uint(line_array[1]) ){
					...
					if(connect_status == "90"){
						...
						bid_num = line_array[27];
						team_str_val = line_array[28];
						team_str_array = team_str_val.split(",");
						margin_val = line_array[29];
						...
						if(item_id_flag != item_id){
							...
							if(price_user_type == "1"){
								...
								if(price_user_id == account_id){
									margin_val = line_array[8];
									marginInfo.htmlText = "已付保证金:" + margin_total_val + "元"  + "<br/>" + "剩余保证金:" + margin_val + "元";
								}
						...
						if(item_id_flag != item_id){
							...
							if(account_id == line_array[11]){
								margin_val = line_array[10];
								marginInfo.htmlText = "已付保证金:" + margin_total_val + "元"  + "<br/>" + "剩余保证金:" + margin_val + "元";
							}
		...
		private function setDescriptionArea(){
			...
			userInfo.htmlText = "用户名:" + user_name + "<br/>" + "拍牌号:" + bid_num;
		...
		private function setControlButtonArea(){
			...
			if(auction_status == "0"){
				...
				if( (item_status == "1" || item_status == "2" 
				   	|| item_status == "3") && pause_flag != "1" 
				   	&& price_user_id != account_id && price_authority == "1"){
					
					plus_fir_mvc.gotoAndStop(1);
					plus_sec_mvc.gotoAndStop(1);
					plus_thi_mvc.gotoAndStop(1);
					plus_fou_mvc.gotoAndStop(1);
					currentPriceFirLabel.textColor = 0x666666;
					currentPriceSecLabel.textColor = 0x666666;
					currentPriceThiLabel.textColor = 0x666666;
					currentPriceFouLabel.textColor = 0x666666;
		...
		private function doPrice(price_str:String):void{
			currentPriceFirButton.gotoAndStop(2);  
			currentPriceFirButton.removeEventListener(MouseEvent.CLICK,currentPriceFirHandler);
			currentPriceSecButton.gotoAndStop(2);  
			currentPriceSecButton.removeEventListener(MouseEvent.CLICK,currentPriceSecHandler);
			...
			currentPriceFouButton.gotoAndStop(2);  
			currentPriceFouButton.removeEventListener(MouseEvent.CLICK,currentPriceFouHandler);
			priceAlertBox = new PriceAlertBox();
			priceAlertBox.price_label.text = price_str;
			priceAlertBox.addEventListener("okEvent",okAuctionCloseHandler);
	       	priceAlertBox.addEventListener("closeEvent",closeAuctionCloseHandler);
			priceAlertBox.addEventListener("cancelEvent",cancelAuctionCloseHandler);
			addChild(priceAlertBox);
			setPriceButtonValue();
		}
		function okAuctionCloseHandler(evt:Event):void {	
			sendMessage("31", (item_id + "&" + priceAlertBox.price_label.text + "&"));
		...
		private function setPriceButtonValue():void{
			...
			if(item_status == "0"){
				...
				currentPriceFirLabel.text = "0";
				currentPriceSecLabel.text = "0";
				currentPriceThiLabel.text = "0";
				currentPriceFouLabel.text = "0";
				currentPriceFirLabel.text = "0";
			}
		}
		...
		private function setPriceDataList():void{
			...
			if(priceDataProvider.length < status_counter_max){
				if(price_user_id == account_id){
					priceDataProvider.addItemAt({flag:"1", 时间:getDateStatusRecorder(), 用户:price_type, 出价:level_fir,拍品:item_index,拍牌:price_user_bid}, new uint(0));
				}else{
		...
		private function setDealDataList():void{  
			...
			if(dealDataProvider.length < status_counter_max){
				if(price_user_id == account_id){
					dealDataProvider.addItemAt({flag:"1", 时间:getDateStatusRecorder(), 用户:price_type, 出价:level_fir,拍品:item_index,拍牌:price_user_bid}, new uint(0));
				}else{
		...
		private function setAuthorityArea(){
			var proxyValue:String;
			if(new uint(item_index) > (proxy_str_array.length-1)){
				proxy_value = "-1"
			}else{
				proxy_value = proxy_str_array[(new uint(item_index)-1)];
			}
			if(proxy_value == "-1"){
				proxyValue = "-";
			}else{
				proxyValue = new uint(proxy_value) + "元";
			} 
			marginInfo.htmlText = "已付保证金:" + margin_total_val + "元"  + "<br/>" + "剩余保证金:" + margin_val + "元";
			price_authority = "0";
			if(team_str_array != null){
				for(var i:int; i<team_str_array.length; i++){
					if(team_str_array[i] == team_id){  
						price_authority = "1";
						break;
					}else{
						price_authority = "0";
				...
				if(price_authority == "1"){
					if(new uint(margin_val) < new uint(item_margin) && price_user_id != account_id){
						price_authority = "0";
					}
				}
			}else{
			...
			if(price_authority == "1"){  
				authorityAndProxy.htmlText = "我的出价权:可以出价<br/>我的代理价:" + proxyValue;
			}else if(price_authority == "0"){
				authorityAndProxy.htmlText = "我的出价权:不可以出价<br/>我的代理价:" + proxyValue;
			}
		}
