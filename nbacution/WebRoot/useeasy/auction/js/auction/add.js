...
function doSubmit(){
	...
	if(!$("#auctionPicType_fir")[0].checked && 
		...
		if($("#auctionPicType_thi")[0].checked){
			if($("#u_file").val() == ""){
				...
				var pic_val = $("#auctionUpLoadImgSec").attr("src");
				if(pic_val.indexOf("nopic") != -1){
					$("#u_file").focus();
					alert("请上传自定义图标！");
					return;
				}
	...
	if(confirm("是否提交该拍卖会")){
		...
		document.forms[0].target = "_self";
	 	document.forms[0].action="auctionManage_save";
		document.forms[0].submit();
	}
